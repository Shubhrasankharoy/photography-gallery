...
export default function ActivityIcon({ action, resourceType, severity, className = "w-5 h-5" }) {
  ...
  const renderIcon = () => {
    if (action === 'photo_uploaded' || action === 'upload') {
      return (
    ...
    if (action === 'photo_deleted' || action === 'event_deleted' || action === 'member_removed') {
      return (
        <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    if (action === 'share_created' || action === 'share_revoked' || resourceType === 'share') {
      return (
        <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    if (action === 'download' || resourceType === 'download') {
      return (
        <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    if (resourceType === 'event' || action.startsWith('event_')) {
      return (
        <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    if (resourceType === 'member' || action.startsWith('member_')) {
      return (
        <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    if (resourceType === 'studio' || action.startsWith('studio_')) {
      return (
        <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">

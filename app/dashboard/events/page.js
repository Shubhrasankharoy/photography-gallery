...
export default function EventsList() {
  ...
  // Derive filtered events list in render phase
    ...
    return (
      <div className="flex min-h-[70vh] items-center justify-center bg-[#F7F7F7] dark:bg-[#181818] transition-colors duration-300">
        <div className="flex flex-col items-center gap-2">
          ...
          <span className="text-sm text-[#8E8E8E] font-medium">Loading event galleries...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <motion.div
      ...
      className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 bg-[#F7F7F7] dark:bg-[#181818] min-h-screen transition-colors duration-300 text-left"
    >
      
      {/* Top Breadcrumb Header */}
      <motion.div
        variants={itemVariants}
        className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-zinc-200/50 pb-6 dark:border-zinc-800/40"
      >
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-zinc-900 dark:text-zinc-50 font-headline">
            Client Event Spaces
          </h1>
          <p className="mt-1.5 text-sm text-[#8E8E8E] font-light">
        ...
        {canCreateEvent && (
          <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
            <Link
              ...
              className="inline-flex items-center justify-center gap-2 rounded-[12px] bg-[#D4AF37] hover:bg-[#E0C55B] px-5 py-2.5 text-xs font-bold text-[#181818] shadow-md hover:shadow-lg transition-all select-none"
            >
      ...
      <AnimatePresence>
        {notification && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 16 }}
            className="fixed bottom-6 right-6 z-50 rounded-[20px] bg-[#181818] border border-zinc-800 px-6 py-4 text-xs font-bold text-white shadow-2xl flex items-center gap-3"
          >
            <svg className="h-4 w-4 text-[#D4AF37] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
      ...
      {/* Search Filter input */}
      <motion.div variants={itemVariants} className="mt-8 relative max-w-md">
        ...
        <input
          ...
          className="w-full rounded-[12px] border border-zinc-200/60 bg-white pl-10 pr-4 py-3 text-xs text-zinc-900 outline-none focus:ring-2 focus:ring-[#D4AF37] dark:border-zinc-800/40 dark:bg-[#262626] dark:text-zinc-100 dark:focus:ring-[#D4AF37] transition-all"
        />
      </motion.div>
      ...
      {filteredEvents.length === 0 ? (
        <motion.div
          variants={itemVariants}
          className="mt-10 rounded-[20px] border border-dashed border-zinc-200/80 p-16 text-center dark:border-zinc-800 bg-white dark:bg-transparent"
        >
          ...
          <h4 className="mt-4 text-sm font-bold text-zinc-850 dark:text-zinc-200">No Event Spaces Located</h4>
          <p className="mt-1.5 text-xs text-[#8E8E8E] font-light max-w-xs mx-auto">
          ...
          {!searchQuery && (
            <div className="mt-6">
              <Link
                href="/dashboard/events/new"
                className="inline-flex rounded-[12px] bg-[#D4AF37] px-6 py-2.5 text-xs font-bold text-[#181818] hover:bg-[#E0C55B] shadow-md transition-all select-none"
              >
                Create Event
              </Link>
            </div>
        ...
        <motion.div
          variants={containerVariants}
          className={`mt-8 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 transition-opacity duration-200 ${
          ...
          {filteredEvents.map((evt) => {
            ...
            return (
              <motion.div
                ...
                className="group relative flex flex-col overflow-hidden rounded-[20px] border border-zinc-200/60 bg-white shadow-[var(--shadow-soft)] hover:shadow-[var(--shadow-lg)] dark:border-zinc-800/40 dark:bg-[#262626] transition-all duration-300"
              >
                ...
                <div className="relative aspect-video w-full bg-zinc-150 dark:bg-zinc-900 overflow-hidden">
                  ...
                  <span className={`absolute top-3 left-3 inline-flex px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider ${
                    evt.visibility === "public"
                      ...
                      : "bg-[#D4AF37]/10 text-[#D4AF37] dark:bg-black/60 dark:text-[#D4AF37]"
                  }`}>
                ...
                <div className="p-5 grow flex flex-col justify-between">
                  <div className="text-left">
                    <h3 className="text-lg font-bold text-zinc-900 dark:text-zinc-50 truncate leading-tight group-hover:text-[#D4AF37] dark:group-hover:text-[#D4AF37] transition-colors font-headline">
                      {evt.eventName}
                    </h3>
                    
                    {/* Date and location */}
                    <div className="mt-2.5 flex flex-col space-y-1 text-xs text-[#8E8E8E] font-light">
                  ...
                  <div className="mt-6 pt-4 border-t border-zinc-150 dark:border-zinc-800 flex items-center justify-between gap-2">
                    ...
                    <div className="flex gap-2">
                      <span
                        ...
                        className="rounded-[12px] border border-zinc-200 hover:bg-zinc-50 dark:border-zinc-800 hover:text-[#D4AF37] dark:hover:bg-zinc-900 p-2.5 text-zinc-600 dark:text-zinc-400 cursor-pointer select-none transition-all"
                        title="Copy Link"
                      >
                        <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
                    ...
                    <div className="flex gap-2">
                      {canEditEvent(evt) && (
                        <Link
                          ...
                          className="rounded-[12px] border border-zinc-200/60 hover:bg-zinc-150/50 dark:border-zinc-800/40 dark:hover:bg-zinc-800 px-3.5 py-2 text-xs font-bold text-zinc-700 dark:text-zinc-350 transition-all hover:text-[#D4AF37] dark:hover:text-[#D4AF37]"
                        >
                          Edit
                        </Link>
                      )}
                      ...
                      {canDeleteEvent(evt) && (
                        <span
                          ...
                          className="rounded-[12px] border border-rose-200/50 hover:bg-rose-50 dark:border-rose-900/30 dark:hover:bg-rose-950/20 px-3.5 py-2 text-xs font-bold text-rose-500 cursor-pointer select-none transition-all"
                        >
                          Delete
                        </span>
                      )}

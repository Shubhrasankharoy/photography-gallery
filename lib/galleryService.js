...
/**
 * Toggles a photo favorite document for a specific actor.
 */
export async function toggleFavorite({ photoId, actorId, actorType, studioId, eventId }) {
  if (!db || !photoId || !actorId) return false;
  const favDocId = `${photoId}_${actorId}`;
  const favRef = doc(db, "photoFavorites", favDocId);
  try {
...
/**
 * Fetch all favorites matching query filters.
 */
export async function getFavorites({ studioId, eventId, actorId, limitCount = 1000, startAfterDoc = null }) {
  if (!db) return [];
  try {
    let q = collection(db, "photoFavorites");
    
    // We construct query filters.
    // If eventId and actorId are both specified, Firestore can utilize the compound index
    if (eventId) {
      q = query(q, where("eventId", "==", eventId));
    }
    if (actorId) {

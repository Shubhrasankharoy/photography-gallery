  ...
  async moveToTrash({
    ...
    if (originalCollection && resourceId) {
      try {
        ...
        console.warn(`Could not mark original document ${originalCollection}/${resourceId} as trashed:`, err);
      }
    }
  ...
  async permanentDelete(trashId, deletedBy) {
    ...
    if (originalCollection && resourceId) {
      try {
        ...
        console.warn(`Firestore document deletion failed for ${originalCollection}/${resourceId}:`, err);
      }
    }

...
export class FirestoreRecoveryProvider extends RecoveryProvider {
      ...
      try {
        ...
        if (resourceType === 'event') {
          try {
            ...
            await syncStudioEvents(studioId);
          } catch (e) {
            console.error("Failed to sync studio events in moveToTrash:", e);
          }
        }
      } catch (err) {
  ...
  async restore(trashId, restoredBy, options = {}) {
    ...
    if (originalCollection && resourceId) {
      ...
      if (trashData.resourceType === 'event') {
        try {
          const { syncStudioEvents } = await import('../studioService');
          await syncStudioEvents(trashData.studioId);
        } catch (e) {
          console.error('Failed to sync studio events on restore:', e);
        }
      }
    }

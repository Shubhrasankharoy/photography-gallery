...
export class TimelineLogger {
  ...
  // --- RECOVERY LOGS ---
  async logResourceTrashed({ studioId, resourceType, resourceId, resourceName, actor }) {
    return await this.service.log({
      studioId,
      resourceType,
      resourceId,
      action: ACTION_TYPES.RESOURCE_TRASHED,
      actorId: actor?.id || actor?.uid || 'system',
      actorName: actor?.name || actor?.displayName || 'System User',
      actorAvatar: actor?.avatar || actor?.photoURL || null,
      title: `${resourceType ? resourceType.toUpperCase() : 'Resource'} Moved to Trash`,
      description: `Moved "${resourceName || resourceId}" to trash.`,
      severity: SEVERITIES.WARNING,
      source: SOURCES.WEB,
      metadata: { resourceType, resourceName }
    });
  }

  async logResourceRestored({ studioId, resourceType, resourceId, resourceName, actor }) {
    return await this.service.log({
      studioId,
      resourceType,
      resourceId,
      action: ACTION_TYPES.RESOURCE_RESTORED,
      actorId: actor?.id || actor?.uid || 'system',
      actorName: actor?.name || actor?.displayName || 'System User',
      actorAvatar: actor?.avatar || actor?.photoURL || null,
      title: `${resourceType ? resourceType.toUpperCase() : 'Resource'} Restored`,
      description: `Restored "${resourceName || resourceId}" from trash.`,
      severity: SEVERITIES.SUCCESS,
      source: SOURCES.WEB,
      metadata: { resourceType, resourceName }
    });
  ...
  async logResourceDeletedForever({ studioId, resourceType, resourceId, resourceName, actor }) {
    return await this.service.log({
      studioId,
      resourceType,
      resourceId,
      action: ACTION_TYPES.RESOURCE_DELETED_FOREVER,
      actorId: actor?.id || actor?.uid || 'system',
      actorName: actor?.name || actor?.displayName || 'System User',
      actorAvatar: actor?.avatar || actor?.photoURL || null,
      title: `${resourceType ? resourceType.toUpperCase() : 'Resource'} Permanently Deleted`,
      description: `Permanently deleted "${resourceName || resourceId}".`,
  ...
  async logTrashEmptied({ studioId, count, actor }) {
    return await this.service.log({
      studioId,
      resourceType: RESOURCE_TYPES.STUDIO,
      resourceId: studioId,
      action: ACTION_TYPES.TRASH_EMPTIED,
      actorId: actor?.id || actor?.uid || 'system',
      actorName: actor?.name || actor?.displayName || 'System User',
      actorAvatar: actor?.avatar || actor?.photoURL || null,
      title: 'Trash Emptied',
      description: `Permanently deleted all ${count} items in trash.`,
      severity: SEVERITIES.ERROR,
      source: SOURCES.WEB,
  ...
  async logBulkRestore({ count, actor }) {
    return await this.service.log({
      resourceType: RESOURCE_TYPES.STUDIO,
      action: ACTION_TYPES.BULK_RESTORE,
      actorId: actor?.id || actor?.uid || 'system',
      actorName: actor?.name || actor?.displayName || 'System User',
      actorAvatar: actor?.avatar || actor?.photoURL || null,
      title: 'Bulk Restore',
      description: `Restored ${count} items from trash.`,
      severity: SEVERITIES.SUCCESS,
      source: SOURCES.WEB,
      metadata: { fileCount: count }
    });
  ...
  async logBulkDelete({ studioId, count, actor, permanent = false }) {
    return await this.service.log({
      studioId,
      resourceType: RESOURCE_TYPES.STUDIO,
      action: permanent ? ACTION_TYPES.RESOURCE_DELETED_FOREVER : ACTION_TYPES.BULK_DELETE,
      actorId: actor?.id || actor?.uid || 'system',
      actorName: actor?.name || actor?.displayName || 'System User',
      actorAvatar: actor?.avatar || actor?.photoURL || null,
      title: permanent ? 'Bulk Permanent Delete' : 'Bulk Move to Trash',
      description: `${permanent ? 'Permanently deleted' : 'Moved to trash'} ${count} items.`,
      severity: permanent ? SEVERITIES.ERROR : SEVERITIES.WARNING,
      source: SOURCES.WEB,
      metadata: { fileCount: count }
    });
  }

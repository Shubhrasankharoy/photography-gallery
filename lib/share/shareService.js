...
export class ShareService {
  ...
  async revokeShare(shareId, actor) {
    ...
    const actorInfo = actor || { uid: oldShare.createdBy || 'system', name: 'User' };

    await recoveryService.moveToTrash({
      resourceType: 'share',
      resourceId: shareId,
      studioId: oldShare.studioId,
      originalCollection: 'sharedResources',
      snapshot: oldShare,
      reason: 'Share link revoked',
      metadata: {
        shareType: oldShare.shareType || 'public',
        deletedFrom: 'share-manager'
      }
    }, actorInfo);

    return { ...oldShare, status: 'trashed' };
  }

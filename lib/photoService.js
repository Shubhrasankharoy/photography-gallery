...
async function createUploadActivity({ studioId, eventId, photoId, uploadedBy, action, fileName, fileSize }) {
  try {
    ...
    timelineLogger.logPhotoUploaded({
      studioId,
      eventId,
      photoCount: 1,
      eventName,
      actor: { id: uploadedBy, name: userDisplayName }
    });
  } catch (err) {
...
export async function afterUpload(photo) {
  try {
    import("./queue/queueService").then(({ queueService }) => {
      queueService.createFaceIndexJob(photo.photoId, photo.eventId, photo.studioId, photo.uploadedBy || 'system');
    });
  } catch (err) {
    console.error("Failed to trigger background face indexing queue:", err);
  }
}
...
export async function afterReplace(photo) {
  try {
    import("./vision/embeddingService").then(({ embeddingService }) => {
      embeddingService.deletePhotoEmbeddings(photo.photoId);
    });
    import("./queue/queueService").then(({ queueService }) => {
      queueService.createFaceIndexJob(photo.photoId, photo.eventId, photo.studioId, photo.uploadedBy || 'system');
    });
  } catch (err) {
    console.error("Failed in afterReplace vision processing:", err);
  }
}
...
export async function deletePhoto(photoId, studioId, userId) {
  ...
  const userSnap = await getDoc(doc(db, "users", userId));
  const user = userSnap.exists() ? userSnap.data() : { uid: userId, displayName: 'User' };
  ...
  await recoveryService.moveToTrash({
    ...
    snapshot: photo,
    reason: 'User deleted photo',
    metadata: {
      originalPath: photo.driveFolderId || photo.url,
      fileSize: photo.fileSize || photo.size || 0,
      deletedFrom: 'gallery',
      storageProvider: photo.storageProvider || 'google-drive'
    }
  }, { uid: userId, name: user.displayName || user.email || 'User' });
...
/**
 * Fetch recent uploads across all events for a studio or photographer (excluding deleted ones).
 */
export async function getRecentUploads(params, limitCount = 4) {
  if (!db) return [];
  const studioId = typeof params === "object" ? params.studioId : null;
  const photographerId = typeof params === "object" ? params.photographerId : params;
...
/**
 * Fetch recent download events for a studio or photographer (sorted by newest first).
 */
export async function getRecentDownloads(params, limitCount = 5) {
  if (!db) return [];
  const studioId = typeof params === "object" ? params.studioId : null;
  const photographerId = typeof params === "object" ? params.photographerId : params;

...
export async function createStudio(userId, studioData) {
  ...
  // Fetch creator user details to seed the members list on the studio doc
  const userRef = doc(db, "users", userId);
  const userSnap = await getDoc(userRef);
  let displayName = "Owner";
  let email = "";
  if (userSnap.exists()) {
    displayName = userSnap.data().displayName || userSnap.data().email || "Owner";
...
export async function acceptInvitation(invitationId, userId) {
  ...
  const defaultPosition = inviteData.role.charAt(0).toUpperCase() + inviteData.role.slice(1);
  ...
  await batch.commit();
  await syncStudioMembers(inviteData.studioId);
}
...
export async function removeStudioMember(studioId, userId) {
  ...
  await syncStudioMembers(studioId);
}
...
export async function transferStudioOwnership(studioId, currentOwnerId, newOwnerId) {
  ...
  batch.update(currentOwnerRef, { role: "admin", position: "Admin" });

  // 2. Promote new owner to owner
  batch.update(newOwnerRef, { role: "owner", position: "Owner" });
...
/**
 * Synchronize the members of a studio directly to the studio document.
 */
export async function syncStudioMembers(studioId) {
  ...
  try {
    ...
    for (const d of querySnapshot.docs) {
      ...
      members.push({
        userId: data.userId,
        role: data.role,
        position: data.position || (data.role ? data.role.charAt(0).toUpperCase() + data.role.slice(1) : "Member"),
    ...
    console.error(`Error syncing studio members for ${studioId}:`, error);
  }
}

/**
 * Synchronize the active events list of a studio directly to the studio document.
 */
export async function syncStudioEvents(studioId) {
  if (!db || !studioId) return;
  try {
    const q = query(
    ...
    console.error(`Error syncing studio events for ${studioId}:`, error);
  }
}

/**
 * Update member's role and position in a studio.
 */
export async function updateStudioMember(studioId, userId, updates) {
  if (!db) throw new Error("Firestore Database is not initialized.");
  const memberRef = doc(db, "studioMembers", `${studioId}_${userId}`);
